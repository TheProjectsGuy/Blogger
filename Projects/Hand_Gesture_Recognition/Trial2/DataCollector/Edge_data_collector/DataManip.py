...
for label_directory in sorted(os.listdir("{r}/".format(r=data_folder_name))):
    ...
    for file_name in os.listdir("{r}/{lb}/".format(r=data_folder_name, lb=label_directory)):
        ...
        img = cv.Canny(img, 20, 30)
        img_flip = cv.Canny(img_flip, 20, 30)
        cv.imshow("Edges of original", img)
        cv.imshow("Edges of flipped", img_flip)
        cv.moveWindow("Edges of original", 50, 400)
        cv.moveWindow("Edges of flipped", 500, 400)
        # Save main image
