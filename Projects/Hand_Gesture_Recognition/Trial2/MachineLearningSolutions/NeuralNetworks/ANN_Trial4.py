...
def softmax():
    ...
    :return:
        Dictionary
            ...
            "derivative" : Softmax derivative function
    """
    ...
    def softmax_derivative(x):
        ...
        :return:
            Derivative of softmax to the input
        """
...
def forward_propagate_deep(params, activations, input, keep_prob=1.0):
    """
    ...
    :param keep_prob: The keep_probability for drop offs in the neural network, by default it's 1 (no dropout) for test
                      cases
    :return:
        A_final, cache
    ...
    for i in range(1, L + 1):
        ...
        if i != L:
            # Implement dropoff in every layer other than the output layer
            dr_cache = np.random.rand(*cache["A" + str(i)].shape) < keep_prob
            cache["A" + str(i)] *= dr_cache
...
# Back propagation for softmax regression
def backward_propagation_grads_softmax(cache, params, activations, Y):
    """
        The backward propagation gradient generator using softmax regression
        :param cache: Output and weghed sum of every neuron of every layer in the neural network
                cache["A" + str(l)] : Output of layer l
                cache["Z" + str(l)] : Weighed sum over the previous layer
        :param params: The Weights and Biases of the neural network
    ...
    L = int(len(params.keys()) / 2)
    m = cache["A0"].shape(1)
    # Calculate the del values on the batch
    del_vals = {}
    activation_derivative = activations[L - 1]()["derivative"]
    del_vals["del" + str(L)] = (-Y)/(m*cache["A" + str(L)]) * activation_derivative(cache["Z" + str(L)])
    for l in range(L-1, 0, -1): # Calculate the del values from layer L-1 to 1
        activation_derivative = activations[l-1]()["derivative"]
        del_vals["del" + str(l)] = np.matmul(params["W" + str(l + 1)].T, del_vals["del" + str(l + 1)]) * \
                                   activation_derivative(cache["Z" + str(l)])
    # Calculate the final gradients
    grads = {}
    # Final backward steps
    for l in range(L, 0, -1):
        grads["dW" + str(l)] = np.matmul(del_vals["del" + str(l)], cache["A" + str(l - 1)].T)
        grads["db" + str(l)] = np.matmul(del_vals["del" + str(l)], np.ones((m, 1)))
    return grads
...
# Back propagation step for softmax regression
def back_propagation_deep_softmax(cache, params, activations, Y, learning_rate=0.01, reg_lambda=0.2):
    """
        Perform one step of backward propagation with softmax regression for multiclass classification
        :param cache: Output and weghed sum of every neuron of every layer in the neural network
                cache["A" + str(l)] : Output of layer l
                cache["Z" + str(l)] : Weighed sum over the previous layer
        :param params: The Weights and Biases of the neural network
    ...
    L = int(len(params.keys()) / 2)
    m = Y.shape[1]
    grads = backward_propagation_grads_softmax(cache, params, activations, Y)
    for l in range(L, 0, -1):   # Adjust parameters from layer L to 1
...
net_config = {
    ...
    "testing": {
        "test_image": "../Data/5_Fingers/Test.jpg",
        "false_examples": "../../DataCollector/Mask_data_collector/Data_Distributions_Backups/"
                          "Data_Distribution_5_Fingers/Data_Distribution_Generated/False",
        "true_examples": "../../DataCollector/Mask_data_collector/Data_Distributions_Backups/"
                         "Data_Distribution_5_Fingers/Data_Distribution_Generated/True",
...
def get_nn_architecture_param(arc):
    """
    To get the architecture of the neural network
    :param arc: Architecture of neural network
    :return:
        tuple(l, f)
        l : array of layers (including input layer)
        f : activation functions of every layer (excluding input layer) as string
    """
    l = arc
    f = []
    for fnc in net_config["nn_arc"]["activations"]:
        if fnc == sigmoid:
            f.append('sigmoid')
        elif fnc == relu:
            f.append('relu')
...
def get_activations_list(acts):
    act_fs = []
    for fnc in acts:
        print("ACTIVATION PARSER : Processing {ac} function".format(ac=fnc))
        if fnc == 'sigmoid':
            act_fs.append(sigmoid)
        elif fnc == 'relu':
            act_fs.append(relu)
        elif fnc == 'tanh':
            act_fs.append(tanh)
...
if __name__ == '__main__':
        ...
        for train_iter_num in range(num_iter):
            ...
            for batch_index in range(len(Y_training_batches)):
                ...
                for epoch_count in range(num_epochs):
                    ...
                    # Back propagation
                    if net_config["data"]["use_softmax"]:
                        params = back_propagation_deep_softmax(cache_mini_batch, params, activations,
                                        Y_training_batch, learning_rate, reg_param_lambda)
                    else:
    ...
    try:
        ...
        print("TEST ERROR : There is an issue with image \"{tm_path}\"".format(
            tm_path=net_config["testing"]["test_image"]
        ))
